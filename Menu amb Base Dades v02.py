...
#Aquí pots posar miCursor.execute('''CREATE TABLE dades IF NOT EXISTS (usuari VARCHAR (50), edat VARCHAR(2))''')
#He canviat el tipus de dada d'edat. Més avall t'explico per què.
#Poso els noms de les taules i les columnes en minúscules, que es com es fa per distingir noms de paraules de codi
#I jo a tot arreu ho trobo amb tres cometes per començar i acabar. No sé si té cap importància.
...
#El resultat del codi --str(age_input)-- crec que deu ser un STRING, però a la BDD has posat que és un INTEGER.
#també pots fer
miCursor.execute("INSERT INTO DADES (Usuari, Edat) VALUES (?, ?)",
((str(user_input),int(age_input))))


miConexion.commit(),
#també convé tancar el cursor 
#miCursor.close()
